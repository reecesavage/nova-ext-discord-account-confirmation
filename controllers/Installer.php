...
function __construct() {
 ...
 public function install() {
    ...
    $expectedLink = 'extensions/nova_ext_discord_account_confirmation/Manage/config';
    ...
    if ( $cat === false ) {
      ...
      $query = $this->ci->db->get_where('menu_items', array('menu_name' => 'Discord Account Confirmation'));
    $item = ($query->num_rows() > 0) ? $query->row() : false;   
      if($item==false){
      $insertItem = $this->ci->menu_model->add_menu_item( [
        'menu_name' => 'Discord Account Confirmation',
        'menu_group' => 0,
        'menu_order' => 0,
        'menu_sim_type' => 1,
...
$expectedLink='extensions/nova_ext_discord_account_confirmation/Manage/discord';
       $query = $this->ci->db->get_where('menu_items', array('menu_name' => 'Discord Account'));
    $item = ($query->num_rows() > 0) ? $query->row() : false;   
      if($item==false){
      $insertItem = $this->ci->menu_model->add_menu_item( [
        'menu_name' => 'Discord Account',
        'menu_group' => 0,
        'menu_order' => 0,
        'menu_sim_type' => 1,
        'menu_link' => $expectedLink,
